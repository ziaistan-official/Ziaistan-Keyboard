...
The characters `\` (unless followed by n or t), `?`, `#`, and `@` do not need to be escaped when writing custom layouts. When writing a layout to be included in the app (in [srcs/layouts](https://github.com/Julow/Ziaistan-Keyboard/tree/master/srcs/layouts)), they are represented by typing `\\`, `\?`, `\#`, and `\@`.
...
Value                  | Meaning
:--------------------- | :------
`config`               | Gear icon; opens Ziaistan Keyboard settings.
...
`switch_forward`       | Change the keyboard layout, as long as Ziaistan Keyboard has multiple keyboard layouts enabled in the settings.
