...
public final class KeyValueParser
{
  ...
  {
    input = input.trim();
    if (input.startsWith(":tiny ")) {
      KeyValue kv = parse(input.substring(6));
      return kv.withFlags(kv.getFlags() | KeyValue.FLAG_TINY_FONT);
    }
    if (input.startsWith(":small ")) {
      KeyValue kv = parse(input.substring(7));
      return kv.withFlags(kv.getFlags() | KeyValue.FLAG_SMALLER_FONT);
    }
    if (input.startsWith(":dim ")) {
      KeyValue kv = parse(input.substring(5));
      ...
      if (input.contains(",")) {
          ...
          try {
              ...
              if (parse_comma(m)) {
                  ...
                  while (parse_comma(m));
                  return KeyValue.makeMacro("", keydefs.toArray(new KeyValue[]{}), 0);
              }
          } catch (ParseError e) {
              // Fallback to string
          }
    ...
    static int parseFlags(String s, Matcher m) throws ParseError
    {
      ...
      for (String f : s.split(","))
      {
        switch (f)
        {
          ...
          case "tiny": flags |= KeyValue.FLAG_TINY_FONT; break;
